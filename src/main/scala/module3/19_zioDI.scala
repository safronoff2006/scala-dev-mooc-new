...
//    }
   ...
   */
  lazy val combined2: ZIO[EmailService with LoggingService with UserService, Throwable, User] =
    getUser zipLeft sendMail
  ...
  lazy val queryAndNotify: ZIO[LoggingService with EmailService with UserService, Throwable, Unit] = for{
    ...
    email <- emailService.makeEmail(user.email, "hello")
